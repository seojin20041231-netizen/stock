...
# --- [사이드바] 백테스트 / 테스트 시점 컨트롤러 ---
st.sidebar.header("⚙️ 테스트 시점 설정")
use_test_mode = st.sidebar.checkbox("과거 시점 테스트 모드 사용", value=True)

target_cutoff_dt = None
ny_tz = pytz.timezone("America/New_York")

if use_test_mode:
    # 어제 날짜 기본 계산 (주말 고려 없이 최근 하루 전)
    default_test_date = datetime.now().date() - timedelta(days=1)
    
    selected_date = st.sidebar.date_input("테스트 날짜 선택", default_test_date)
    # 본장 시작 1시간 전 = 08:30 AM EST
    selected_time = st.sidebar.time_input("시뮬레이션 시각 (미국 동부시 EST)", datetime.strptime("08:30", "%H:%M").time())
    
    # 통합 Datetime 생성 (미국 뉴욕 타임존 설정)
    naive_dt = datetime.combine(selected_date, selected_time)
    target_cutoff_dt = ny_tz.localize(naive_dt)
    st.sidebar.info(f"📍 테스트 시점:\n{target_cutoff_dt.strftime('%Y-%m-%d %H:%M:%S %Z')}")
...
def get_stock_data(ticker_symbol, cutoff_dt=None):
    ticker = yf.Ticker(ticker_symbol)
    info = ticker.info
    
    # 7일치 5분봉 & 6개월치 일봉 수집
    hist_5m = ticker.history(period="7d", interval="5m", prepost=True)
    hist_daily = ticker.history(period="6mo", interval="1d")
    
    if hist_5m.empty:
        return None

    # 타임존 통일 (America/New_York)
    if hist_5m.index.tz is None:
        hist_5m.index = hist_5m.index.tz_localize('UTC').tz_convert(ny_tz)
    else:
        hist_5m.index = hist_5m.index.tz_convert(ny_tz)

    # --- [백테스트 시점 필터링 핵심 로직] ---
    if cutoff_dt is not None:
        # 기준 시점 이전 데이터만 필터링
        filtered_5m = hist_5m[hist_5m.index <= cutoff_dt]
        if filtered_5m.empty:
            return None
        
        last_day = cutoff_dt.date()
        current_price = filtered_5m['Close'].iloc[-1] # 기준 시점의 마감가
        ref_timestamp = cutoff_dt.timestamp()
    else:
        filtered_5m = hist_5m
        last_day = filtered_5m.index[-1].date()
        current_price = info.get('currentPrice', info.get('regularMarketPrice', filtered_5m['Close'].iloc[-1]))
        ref_timestamp = time.time()

    # 해당 타겟 날짜의 인트라데이 데이터 추출
    today_hist = filtered_5m[filtered_5m.index.date == last_day].copy()
    if today_hist.empty:
        return None
    ...
    # 전일 종가 보정 (테스트 모드일 때는 과거 일봉의 마지막 종가 활용)
    if cutoff_dt is not None and not past_daily.empty:
        prev_close = past_daily['Close'].iloc[-1]
    else:
        prev_close = info.get('previousClose', past_daily['Close'].iloc[-1] if not past_daily.empty else current_price)
    ...
    if not today_hist.empty and today_volume > 0:
        v = today_hist['Volume']
        ...
        if len(today_hist) >= 5: # 프리장 초반 고려 수치 완화
    ...
    for n in news_items:
        ...
        # 테스트 기준 시점으로부터 24시간 이전 ~ 기준 시점 사이 발송 뉴스
        if 0 <= (ref_timestamp - pub_time) <= 86400:
            has_today_news = True
...
if use_test_mode and target_cutoff_dt:
    st.warning(f"🧪 **백테스트 모드 가동 중**: 기준 시점 = `{target_cutoff_dt.strftime('%Y-%m-%d %H:%M EST')}`")

target_ticker = st.text_input("프리마켓 급등 종목 티커 입력 (예: CISS, FFIE)", "CISS").upper()

if target_ticker:
    with st.spinner("지정 시점 데이터 및 인트라데이 수급 판독 중..."):
        data = get_stock_data(target_ticker, target_cutoff_dt)
        
    if data is None:
        st.error("지정한 시점의 데이터(또는 프리마켓 거래 내역)를 불러올 수 없습니다. 날짜/시간을 확인해 주세요.")
    else:
        ...
        with col2:
            ...
            st.markdown(f"<div style='text-align: right; color: #888; font-size: 14px;'>시점 대비 등락: {data['change']:.2f}%</div>", unsafe_allow_html=True)
        ...
        if data['news_sentiment'] == "BAD":
            ...
            verdict_desc = "유상증자(Offering), 상폐 경고 등 악재 뉴스가 감지되었습니다."
            ...
            verdict_desc = "명분이 부족하거나 수급이 약하며 프리장 평단가(VWAP) 아래로 밀렸습니다."
            ...
            verdict_desc = "현재 VWAP 아래입니다. 다시 회복하기 전까진 접근 대기하세요."
            ...
            verdict_desc = "호재 + 거래량 + VWAP 지지 + 정배열. 눌림목 공략 대상입니다."
        ...
        with g1_c1:
            st.markdown("<div class='metric-label'>시점 기준 누적 거래량</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='metric-val val-orange'>{int(data['pm_volume']/1000):,} K</div>", unsafe_allow_html=True)
        with g1_c2:
            st.markdown("<div class='metric-label'>20일 평균 대비(배)</div>", unsafe_allow_html=True)
        ...
        with g1_c4:
            st.markdown("<div class='metric-label'>분봉 수급 (OBV)</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='metric-val' style='color: {data['intra_obv_color']};'>{data['intra_obv_status']}</div>", unsafe_allow_html=True)

        # 📰 [뉴스 박스]
        news_title_str = "<br>".join([f"- {t}" for t in data['news_titles']]) if data['has_news'] else "시점 기준 24시간 내 올라온 영문 뉴스/공시가 없습니다."
        st.markdown(f"""
        <div class="news-box">
            <h5 style="margin-top: 0px; margin-bottom: 10px; color: #E0F7FA;">📰 해당 시점 뉴스/공시 체크</h5>
        ...
        with g2_c1:
            st.markdown("<div class='metric-label'>기준 지지선 (VWAP)</div>", unsafe_allow_html=True)
        ...
        with g2_c3:
            ...
            st.markdown("<div class='metric-label' style='margin-top:10px;'>최대 거래량 매물대</div>", unsafe_allow_html=True)
